...
function Photo({ src, alt, className = '' }: { src: string; alt: string; className?: string }) {
  return (
    <div className={`relative h-full rounded-sm overflow-hidden zoom-frame ${className}`}>
  ...
  return (
    <section className="relative bg-card py-24 md:py-32 overflow-hidden">
      ...
      <Container className="relative">
        ...
        {/* Editorial grid. Text tiles set the row heights; photos fill whatever
            height they are given (absolute + object-cover), so a tall photo can
            never stretch a row and unbalance the page. */}
        <div className="grid grid-cols-2 md:grid-cols-12 md:auto-rows-[minmax(200px,auto)] gap-4 md:gap-5">
          {/* row 1-2 */}
          <Reveal className="col-span-2 md:col-span-5 md:row-span-2">
            <Photo src={p1} alt="Jessica" className="min-h-[360px] md:min-h-0" />
          </Reveal>
          <Reveal delay={0.05} className="col-span-2 md:col-span-4 sky rounded-sm p-7 md:p-8 flex items-end">
            <p className="font-display text-2xl md:text-[1.7rem] text-ivory leading-snug">“{q[0].text}”</p>
          </Reveal>
          ...
          {/* row 3-4 */}
          <Reveal delay={0.16} className="col-span-2 md:col-span-4 md:row-span-2">
            <Photo src={p2} alt="A themed tea party table from one of Jessica's events" className="min-h-[300px] md:min-h-0" />
          </Reveal>
          ...
          <Reveal delay={0.2} className="col-span-1 md:col-span-5 bg-background border border-border/70 rounded-sm p-6 md:p-7 flex items-center">
            <p className="font-display text-xl md:text-2xl text-forest leading-snug">“{q[8].text}”</p>
          </Reveal>
          <Reveal delay={0.22} className="col-span-1 md:col-span-4">
            <Photo src={p3} alt="A gathering of women" className="min-h-[220px] md:min-h-0" />
          </Reveal>
          <Reveal delay={0.24} className="col-span-1 md:col-span-4 bg-background border border-border/70 rounded-sm p-6 flex items-center">
            <p className="font-display text-xl md:text-2xl text-forest leading-snug">“{q[3].text}”</p>
          </Reveal>

          {/* row 5: full-width closing line */}
          <Reveal delay={0.26} className="col-span-2 md:col-span-12 sky rounded-sm p-8 md:p-10 flex items-center justify-between gap-6">
            <p className="font-display text-2xl md:text-3xl text-ivory leading-snug max-w-4xl">“{q[9].text}”</p>
